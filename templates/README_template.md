This readme file was generated on [YYYY-MM-DD] by [NAME]

# [TITLE OF DATASET]

## Contact(s)

Name: [Name of data custodian]

Email: [Email of data custodian]

Institution: ACCESS-NRI

## Data Access and Sharing

### Licensing/restrictions

[List any licenses and/or restrictions placed on the data]

### Other locations

[Links to other publicly accessible locations of the data]

### Links/relationships

[Detail any links/relationships to other datasets]

### Recommended citation

[Recommended citation for this dataset]

## Data and Files

### File List

[List of all files/folders contained in the dataset, and a description]

### Data Specific Information

[List details for each file without embedded metadata]

## Methodological information

[Detail workflow and processing steps to replicate data]